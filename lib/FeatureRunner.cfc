...
<cfcomponent output="false">
  ...
  <cffunction name="runFeature" output="false">
    ...
    <cfloop list="#story#" index="line" delimiters="#chr(10)#">
      <cfset matchData = reFindNoCase("^\s*Feature:(.+)$", line, 1, true)>
      <cfif matchData.pos[1]>
        <cfset _report.enterBlock(mid(line, matchData.pos[2], matchData.len[2]))>
      </cfif>
      <cfset matchData = reFindNoCase("^\s*Scenario:(.+)$", line, 1, true)>
      <cfif matchData.pos[1]>
        <cfset _report.addExample("pass", mid(line, matchData.pos[2], matchData.len[2]))>
      </cfif>
    </cfloop>

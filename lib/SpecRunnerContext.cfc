<cfcomponent extends="Base" output="false"><cfscript>
  ...
  function init(spec) {
    ...
    $keywords = "INIT,RUNSPEC,RUNSPECFILE,RUNSPECSUITE,$,$EVAL,STUB,MOCK,FAIL,PEND,THIS,$CFSPEC,CFSPECBINDINGS,GETMATCHERS,ENSURENOEXCEPTIONSAREPENDING,ENSURENODELAYEDMATCHERSAREPENDING,FLAGEXPECTATIONENCOUNTERED,GETANDRESETPENDINGEXCEPTION,SETPENDINGEXCEPTION,GETJAVALOADER,GETINFLECTOR,FLAGDELAYEDMATCHER,SIMPLEMATCHER,GETSIMPLEMATCHEREXPRESSION,REGISTERMATCHER,DESCRIBESTARTTAG,BEFOREALLSTARTTAG,BEFOREALLENDTAG,BEFORESTARTTAG,BEFOREENDTAG,ITSTARTTAG,ITENDTAG,AFTERSTARTTAG,AFTERENDTAG,AFTERALLSTARTTAG,AFTERALLENDTAG,DESCRIBEENDTAG";
    $current = "0";
    $context = [];
    $contextStatus = [];
    $targets = [];
  ...
  function ensureNoExceptionsArePending() {
    if (hasExpectedException()) rethrow(getExpectedException());
  }

  function ensureNoDelayedMatchersArePending() {
